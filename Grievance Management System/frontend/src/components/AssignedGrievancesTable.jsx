...
export default function AssignedGrievancesTable() {
    ...
    const fetchAssignedGrievances = useCallback(async () => {
        if (!id || !jwt) return;
        try {
            const response = await fetch(`http://localhost:8080/api/assignments/assignee/${id}`, {
                method: 'GET',
                headers: {
    ...
    }, [fetchAssignedGrievances]); // Dependencies array
